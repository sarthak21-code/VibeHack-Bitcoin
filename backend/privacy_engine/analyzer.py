...
def compare_candidates(
    ...
    # Quantified fee baseline: the cheapest valid candidate is the reference
    # point every other candidate's fee cost is measured against, so trade-offs
    # can be stated in concrete sats rather than only qualitative language.
    cheapest_fee_sats = min(r.transaction.fee_sats for r in analysis_results)

    for res in analysis_results:
        pros: list[str] = []
        cons: list[str] = []

        fee_delta_sats = res.transaction.fee_sats - cheapest_fee_sats
        is_cheapest = fee_delta_sats == 0

        if is_cheapest:
            pros.append(f"Lowest fee among the generated candidates ({res.transaction.fee_sats:,} sats).")
        else:
            cons.append(
                f"Costs {fee_delta_sats:,} more sats in fees than the cheapest candidate "
                f"({res.transaction.fee_sats:,} vs {cheapest_fee_sats:,} sats)."
            )
        ...
        tradeoffs.append(
            CandidateTradeoff(
                ...
                cons=cons,
                metrics={
                    "fee_sats": res.transaction.fee_sats,
                    "fee_delta_vs_cheapest_sats": fee_delta_sats,
                    "is_cheapest_fee": is_cheapest,
                    "input_count": res.transaction.input_count,
                    "clusters_count": res.privacy.clusters_count,
                    "clusters_merged": res.privacy.clusters_merged,
                    "rare_utxo_consumed": res.future_optionality.rare_utxo_consumed,
                    "change_created": res.transaction.change_created,
                },
            )
        )
