...
from api import app, FinalizeRequest, BroadcastRequest
...
class TestFinalizeAndBroadcast(unittest.TestCase):
    ...
    def test_unsigned_psbt_cannot_finalize(self):
        ...
        self.assertTrue(res["candidate_a"]["valid"])
    ...
    def test_broadcast_rejects_empty_and_short_hex(self):
        ...
        self.assertEqual(response.status_code, 422)  # Pydantic min_length=10

    def test_broadcast_rejects_malformed_hex(self):
        """Broadcast must reject non-hex or non-transaction bytes."""
        response = self.client.post(
            "/broadcast",
            json={"raw_transaction_hex": "00112233445566778899aabbccddeeff"},
        )
        self.assertEqual(response.status_code, 400)
        self.assertIn("Invalid transaction hex", response.json()["detail"])

    def test_network_guard_signet(self):
        """Wallet network must be SIGNET."""
        self.assertEqual(self.wallet.network(), bdk.Network.SIGNET)
